...
let cookiesArr = [],
    cookie = '',id='585324',activityId='fb64cd8222a74c0f96557c2d97547e76';
if ($.isNode()) {
...
!(async () => {
    ...
    $.shareUuid = 'd17d52edd3b94af7bb12b01fb1421f1a'
...
function followShop() {
    return new Promise(resolve => {
        let body = `pin=${encodeURIComponent($.myPingData.secretPin)}&activityId=${activityId}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid}&taskType=23&taskValue=1000000745`
...
function getDrawRecordHasCoupon() {
    return new Promise(resolve => {
        let body = `activityId=${activityId}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}`
...
function saveTask() {
    return new Promise(resolve => {
        let body = `activityId=${activityId}&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid}&taskType=2&taskValue=2389282`
...
async function startDraw(type) {
    return new Promise(resolve => {
        let body = `activityId=${activityId}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}&type=${type}`
        $.post(taskPostUrl('/dingzhi/dz/openCard/startDraw', body,`https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/805745?activityId=${activityId}&shareUuid=${$.shareUuid}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function getHtml() {
    return new Promise(resolve => {
        $.get({
            url: `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/1263438?activityId=${activityId}&shareUuid=${$.shareUuid}&adsource=null&shareuserid4minipg=Pzucjj0+eJIsuMxb3FQreU7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w==&shopid=1000000745&lng=114.062626&lat=29.541499&sid=5fa6c7778669e4865e2e7e7ba5ea098w&un_area=4_48201_54794_0`,
            headers: {
                'User-Agent': UA,
                'Host':'lzdz1-isv.isvjcloud.com',
...
function getActorUuid() {
    return new Promise(resolve => {
        $.post({
            ...
            body: `activityId=${activityId}&pin=${encodeURIComponent($.myPingData.secretPin)}&pinImg=https%3A%2F%2Fimg10.360buyimg.com%2Fimgzone%2Fjfs%2Ft1%2F7020%2F27%2F13511%2F6142%2F5c5138d8E4df2e764%2F5a1216a3a5043c5d.png&nick=${encodeURIComponent($.myPingData.nickname)}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid}`,
            headers: {
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Host':'lzdz1-isv.isvjcloud.com',
                'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
